...
class SMBMap():
    ...
    def login(self, host, username, password, domain):
        try:
            ...
            if self.smbconn[host].isGuestSession() > 0:
                print (f'[+] Guest SMB session established on {host}...')
            else:
                print (f'[+] User SMB session establishd on {host}...')
            return True
    ...
    def login_rpc_hash(self, host, username, ntlmhash, domain):
        ...
        try:
            ...
            if self.smbconn[host].isGuestSession() > 0:
                print(f'[+] Guest RPC session established on {host}...')
            else:
                print(f'[+] User RPC session establishd on {host}...') 
            return True
    ...
    def start_file_search(self, host, pattern, share, search_path):
        ...
        try:
            ...
            print(f'[+] Job {job_name} started on {host}, result will be stored at {tmp_dir}\{job_name}.txt')
        except Exception as e:
            print(e)
            print(f'[!] Job creation failed on host: {host}') 
            ...
            try:
                for job in self.jobs.keys():
                    ...
                    if 'not locked' ==  result.strip() and isItThere.strip() == 'ImHere':
                        ...
                        print(f'[+] Job {counter} of {num_jobs} completed on {self.jobs[job]["host"]}...')
    ...
    def output_shares(self, host, lsshare, lspath, verbose=True, depth=255):
        ...
        for share in shareList:
            ...
            try:
                ...
                print(f'\t{share.ljust(50)}\tREAD, WRITE')
                canWrite = True
                try:
                    self.remove_dir(host, share, root)
                except:
                    print(f'\t[!] Unable to remove test directory at \\\\{host}\\{share}{root}, plreae remove manually')
            except Exception as e:
                #print e
            ...
            if canWrite == False:
                ...
                if readable:
                    print(f'\t{share.ljust(50)}\tREAD ONLY')
                else:
                    error += 1
                ...
                if error == 0:
                    ...
                    if self.list_files and not self.recursive:
                        if lsshare and lspath:
                            if self.pattern:
                                print(f'\t[+] Starting search for files matching \'{self.pattern}\' on share {lsshare}.')
            ...
            if error > 0 and verbose:
                print(f'\t{share.ljust(50)}\tNO ACCESS') 
                            ...
                            if smbItem.is_directory() <= 0:
                                if self.pattern:
                                    ...
                                    if fileMatch:
                                        ...
                                        print(f'\t[+] Match found! Downloading: {ntpath.normpath(dlThis)}')
                                        self.download_file(host, dlThis, False)
                            if verbose:
                                print(f'\t{isDir}{readonly}--{readonly}--{readonly}-- {str(filesize).rjust(width)} {date}\t{filename}')
                ...
                if item.is_directory() <= 0:
                    if self.pattern:
                        ...
                        if fileMatch:
                            ...
                            dlThis = dlThis.replace('/','\\')
                            print(f'\t[+] Match found! Downloading: {dlThis}')
                            self.download_file(host, dlThis, False)
                if verbose:
                    print(f'\t{isDir}{readonly}--{readonly}--{readonly}-- {str(filesize).rjust(width)} {date}\t{filename}')
            return True
        ...
        try:
            ...
            if 'STATUS_ACCESS_DENIED' in str(e):
                ...
                if not verbose:
                    ...
                    indent = ''
                print(f'{indent}[!] Error retrieving file {filename}, sharing violation')
                out.close()
    ...
    def delete_file(self, host, path, verbose=True):
        ...
        try:
            ...
            if 'STATUS_ACCESS_DENIED' in str(e):
                ...
                print(f'[!] Error deleting file {share}{path}{filename}, unkown error')
            print(f'[!], {e}')
        except Exception as e:
            print(f'[!] Error deleting file {share}{path}{filename}, unkown error')
            print(f'[!], {e}')
    ...
    def upload_file(self, host, src, dst):
        ...
        if os.path.exists(src):
            ...
            try:
                ...
                print(f'[!], {e}')
                print(f'[!] Error uploading file, you need to include destination file name in the path') 
            upFile.close()
        else:
    ...
    def get_version(self, host):
        try:
            ...
            print(f"Version Major: {resp['InfoStruct']['ServerInfo102']['sv102_version_major']}")
            print(f"Version Minor: {resp['InfoStruct']['ServerInfo102']['sv102_version_minor']}")
            print(f"Server Name: {resp['InfoStruct']['ServerInfo102']['sv102_name']}")
            print(f"Server Comment: {resp['InfoStruct']['ServerInfo102']['sv102_comment']}")
            print(f"Server UserPath: {resp['InfoStruct']['ServerInfo102']['sv102_userpath']}")
            print(f"Simultaneous Users: {resp['InfoStruct']['ServerInfo102']['sv102_users']}")
        except Exception as e:
...
if __name__ == "__main__":
    ...
    for host in mysmb.hosts.keys():
        if args.file_content_search:
            counter += 1
            print(f'[+] File search started on {counter} hosts...this could take a while')
        ...
        try:
            ...
            if not args.dlPath and not args.upload and not args.delFile and not args.list_drives and not args.command and not args.file_content_search:
                print(f"[+] IP: {host}:{mysmb.hosts[host]['port']}\tName: {mysmb.hosts[host]['name'].ljust(50)}")
                print(f"\tDisk{' '.ljust(50)}\tPermissions")
                print(f"\t----{' '.ljust(50)}\t-----------")
